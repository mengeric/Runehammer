...
// TestExpressionParserDetailed 详细测试表达式解析器
func TestExpressionParserDetailed(t *testing.T) {
	Convey("表达式解析器详细测试", t, func() {
		...
		Convey("表达式解析", func() {
			...
			Convey("中文函数替换", func() {
				result, err := parser.ParseExpression("求和(scores)")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "Sum(scores)")
				
				result, err = parser.ParseExpression("现在")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "Now()")
				
				// 测试新的时间戳函数
				result, err = parser.ParseExpression("当前毫秒")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "NowMillis()")
				
				result, err = parser.ParseExpression("毫秒时间戳")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "NowMillis()")
			})
			
			Convey("中文操作符替换", func() {
				result, err := parser.ParseExpression("age 大于 18 且 status 等于 'active'")
				So(err, ShouldBeNil)
		...
		Convey("动作表达式解析", func() {
			...
			Convey("基本赋值", func() {
				result, err := parser.ParseAction("result.score", "求和([80, 90, 75])")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "result[\"score\"] = Sum([80, 90, 75])")
			})
			
			Convey("中文字段映射", func() {
				result, err := parser.ParseAction("结果.等级", "'VIP'")
				So(err, ShouldBeNil)
				So(result, ShouldEqual, "result[\"level\"] = 'VIP'")
			})
		...
		Convey("辅助函数测试", func() {
			...
			Convey("布尔字面量检查", func() {
				...
				So(parser.isBooleanLiteral("真"), ShouldBeTrue)
				So(parser.isBooleanLiteral("假"), ShouldBeTrue)
				So(parser.isBooleanLiteral("maybe"), ShouldBeFalse)
			})
			
			Convey("布尔字面量标准化", func() {
				So(parser.normalizeBooleanLiteral("真"), ShouldEqual, "true")
				So(parser.normalizeBooleanLiteral("假"), ShouldEqual, "false")
