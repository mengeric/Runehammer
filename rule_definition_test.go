...
func TestRuleDefinition(t *testing.T) {
	Convey("规则定义标准测试", t, func() {
		...
		Convey("常量定义", func() {
			...
			Convey("操作符常量", func() {
				// 比较操作符
				So(OpEqual, ShouldEqual, "==")
				So(OpNotEqual, ShouldEqual, "!=")
				So(OpGreaterThan, ShouldEqual, ">")
				So(OpLessThan, ShouldEqual, "<")
				So(OpGreaterThanOrEqual, ShouldEqual, ">=")
				So(OpLessThanOrEqual, ShouldEqual, "<=")
				
				// 逻辑操作符
				So(OpAnd, ShouldEqual, "and")
				So(OpOr, ShouldEqual, "or")
				So(OpNot, ShouldEqual, "not")
				
				// 集合操作符
				So(OpIn, ShouldEqual, "in")
				So(OpNotIn, ShouldEqual, "notIn")
				So(OpContains, ShouldEqual, "contains")
				So(OpMatches, ShouldEqual, "matches")
				So(OpBetween, ShouldEqual, "between")
			})
			
			Convey("条件类型常量", func() {
				So(ConditionTypeSimple, ShouldEqual, "simple")
				So(ConditionTypeComposite, ShouldEqual, "composite")
				So(ConditionTypeExpression, ShouldEqual, "expression")
				So(ConditionTypeFunction, ShouldEqual, "function")
			})
			
			Convey("动作类型常量", func() {
				So(ActionTypeAssign, ShouldEqual, "assign")
				So(ActionTypeCalculate, ShouldEqual, "calculate")
				So(ActionTypeInvoke, ShouldEqual, "invoke")
				So(ActionTypeAlert, ShouldEqual, "alert")
				So(ActionTypeLog, ShouldEqual, "log")
				So(ActionTypeStop, ShouldEqual, "stop")
			})
		})
	})
}
